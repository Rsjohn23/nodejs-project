...
async function retrieveLast20Records() {
  const client = new MongoClient(url, { useNewUrlParser: true, useUnifiedTopology: true });
  ...
  try {
    ...
    // Access the desired database and collection
    const db = client.db(dbName);
    const collection = db.collection('dbase'); // Change 'your_collection_name' to your actual collection name

    // Retrieve the last 20 records from the collection
    const records = await collection.find().sort({ _id: -1 }).limit(20).toArray();
    console.log('Last 20 records:', records);
    results = records;
  } catch (error) {
    console.error('Error:', error);
  } finally {
...
app.get('/last-20-records', async (req, res) => {
